...
const Hero = () => {
  ...
  const handleGoogleLoginClick = async () => {
    ...
    try {
      ...
      if (error && error.message !== "User cancelled login" && error.message !== "Popup closed") {
          ...
          {session ? (
            ...
            <>
              ...
              <button
                ...
                className="group inline-flex items-center gap-3 rounded-full border border-[#FAF8F5]/25 bg-[#0D0D12]/70 px-7 py-4 text-base font-semibold text-[#FAF8F5] shadow-[0_0_24px_rgba(10,10,14,0.4)] backdrop-blur-sm transition-all duration-300 hover:scale-[1.02] hover:border-[#22C55E]/40 hover:bg-[#16161C] disabled:cursor-not-allowed disabled:opacity-70"
              >
                <GoogleIcon className="h-5 w-5" />
                {googleLoading
                  ? t("common.loading")
                  : t("auth.continueWithGoogle", { defaultValue: "Fortsätt med Google" })}
              </button>
            </>
          )}
        </div>
